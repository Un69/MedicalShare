...
public class ServiceManager {
  ...
  /**
   * @notice: must use @Qualifier("MedicalShareService") with @Autowired to get this Bean
   */
  @Bean("MedicalShareService")
  public Map<String, MedicalShareService> initMedicalShareServiceManager() throws Exception {
    Map<String, MedicalShareService> serviceMap = new ConcurrentHashMap<>(this.hexPrivateKeyList.size());
    for (int i = 0; i < this.hexPrivateKeyList.size(); i++) {
    	...
    	MedicalShareService medicalShareService = new MedicalShareService();
    	medicalShareService.setAddress(this.config.getContract().getMedicalShareAddress());
    	medicalShareService.setClient(this.client);
    	...
    	medicalShareService.setTxProcessor(txProcessor);
    	serviceMap.put(userAddress, medicalShareService);
    }
    log.info("++++++++MedicalShareService map:{}", serviceMap);
    return serviceMap;
  }
}
